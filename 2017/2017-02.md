...
上午物联网研究院开会从九点开到十一点四十五。中午跟余天荣交流教学督导计划的事情，跟杨树元交流物联网研究院的业务和产品问题。下午桌子上趴着歇了一会。下午断网，在手机上看了看 51CTO 学院最近发的一些课程，跟张洋洋了解了 51CTO 学院的企业订单情况。把师大的门禁卡激活了，把师大的饭卡激活了。给学生远程解决了一个 Git 凭证的问题。Git 凭证存储了账户，修改了账户，不能用修改的账户登录，因为，旧的账户已经被存储了。另外，凭证的配置在 .gitconfig 中编辑不了，只能在 Git config -l 中查看到。解决办法，增加了一个 cach 凭证处理程序，两个凭证处理程序同时存在，测试 git push 可以工作了。整理了 sample 仓库的目录，并修改了 seIDE 中自动化脚本的链接。看了几集姚环老师的 HTML & CSS 课程。
